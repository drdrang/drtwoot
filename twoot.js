...
function htmlify(body, entities, extentities) {
  ...
  $.each(urls, function(i, u) {
    if (u.display_url != null) {
      ...
      // Droplr image
      else if (u.expanded_url.slice(0, 14) == 'http://d.pr/i/') {
        link = '<br /><a href="' + u.expanded_url + '+">' + '<img class="inline" src="' + u.expanded_url + '+" /></a><br />';
      }
      // Regular link
      else {
      ...
      $.getJSON(CGI, {url:mentionsURL}, function(mentions){
        ...
        $.each(home, function(i, item){
          if($("#msg-" + item.id_str).length == 0) { // <- fix for twitter caching which sometimes have problems with the "since" parameter
            ...
            var show = 1;
            var mlen = MUTESTRINGS.length;
            for (m=0; m<mlen; m++) {
              mstring = MUTESTRINGS[m].toLowerCase();
              if (theText.toLowerCase().indexOf(mstring) >= 0) {
                show = 0;
              }
            }
            if (show) {
